...
- **Simple** is chat-first: the statusbar, profile rail, file browser and review panes, the technical tool-call view, inline code diffs, and the Artifacts / Scheduled jobs rows rest out of the way (Capabilities and Messaging stay — they are how you set Hermes up) and the terminal folds to its rail; thinking starts collapsed; session rows show the title, a preview and when they were last active. The titlebar keeps Settings and the layout editor. The layout shelf keeps *Basic*, *Focus* and your saved layouts; the tooling decks (*Default*, *Terminal deck*, *Quad*) are Advanced.
